...
serve(async (req) => {
  ...
  try {
    ...
    // Walidacja
    if (!targetNick || amount === undefined || amount === null) {
      return new Response(
    ...
    // Dodaj street cred
    const newAmount = (targetChar.street_cred || 0) + amount
    const { error: updateError } = await supabaseAdmin
      .from('postacie')
      .update({ street_cred: newAmount })
      .eq('id', targetChar.id)
    ...
    const { error: auditError } = await supabaseAdmin
      ...
      .insert({
        ...
        akcja: 'admin_give_street_cred',
        szczegoly: {
          amount: amount,
          old_balance: targetChar.street_cred || 0,
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: `Added ${amount} street cred to ${targetChar.nick}`,
